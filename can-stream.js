...
function canStream(canStreamInterface) {

	var canStreamWithInterface = {
		toComputeFromEvent: function(observable, eventName){
			var handler,
				lastSet;
		...
		toStreamFromProperty: function(obs, propName) {
			return canStreamInterface.toStream(compute(obs, propName));
		},
		toStreamFromEvent: function() {
			var obs = arguments[0];
			...
			if(arguments.length === 2) {
				...
				internalCompute = this.toComputeFromEvent(obs,  arguments[1]);
			...
			if(arguments.length === 1) {
				...
				if(eventNameOrPropName.indexOf(" ") === -1) {
					...
					if(eventNameOrPropName.indexOf(".") === 0) {
						//starts with a dot
						return canStreamWithInterface.toStreamFromProperty(obs, eventNameOrPropName.slice(1)); //toStream(obj, "tasks")
					}
					else {
						return canStreamWithInterface.toStreamFromEvent(obs, eventNameOrPropName); //toStream( obj, "close")
					}
				}
				else {
					var splitEventNameAndProperty = eventNameOrPropName.split(" ");
					return canStreamWithInterface.toStreamFromEvent(obs, splitEventNameAndProperty[0].slice(1), splitEventNameAndProperty[1]);  //toStream(obj, "tasks add")
				}
			}
			return undefined;
		},
